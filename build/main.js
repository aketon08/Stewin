...
class Game {
    ...
    generateMap() {
        /* this.loadingStatus = "Generating map" */ return new Promise((resolve)=>{
            ...
            this.map = new (0, _map.MapGenerator)(this.mapDimensions, this.ctx, 100, 0.4, this.visualiseMap, 0, this);
            this.map.initMap();
            let run = setInterval(()=>{
...
class MapGenerator {
    ...
    constructor(mapDimensions, ctx, maxWalkers, fillPercentage, visualise, waitTime, game){
    ...
    draw(ctx, mapOffset = new (0, _utils.Vec2D)(0, 0)) {
        ...
        for(let i = 0; i < this.mapDimensions.x; i++)for(let j = 0; j < this.mapDimensions.y; j++){
            ...
            let dimensions;
            if (this.generated) {
                position = new (0, _utils.Vec2D)(Math.floor(this.game.playerSize.x * j) + mapOffset.x, Math.floor(this.game.playerSize.y * i) + mapOffset.y);
    ...
    zoom() {
        ...
        for(let i = 0; i < newMap.length; i++){
            for(let j = 0; j < newMap[i].length; j++)if (newMap[i][j] == TileType.Floor) {
                ...
                if (closeToFloor > 1 && Math.random() < 0.07) newMap[i][j] = TileType.Floor;
            }
        }
        this.map = newMap;
